...
Now it's the time to me walking you through the new features and bug fixes of the new version of KAPLAY. 

But before driving into the features, let's recap the installation process for installing the **@next** KAPLAY version:
...
New polygonal helpers and functions just landed KAPLAY! `createRegularPolygon()` and `createStartPolygon()`, they will create the necessary `Vec2` points for making a shape with `polygon()`, like a **Hexagon** (still not Clickery) or a **star**.
...
It's great for fast testing shapes, enjoy them!
...
Now you can define parameters inside a text tag for dynamically change how the style looks, depending on a variable. Imagine all the \[color=rainbow\] possibilities \[/color\]:
...
In this update, Vertical Sweep and Prune, Quadtree (\*1) and the possibility of choose the broad phase algorithm was added. You can choose it using `KAPLAYOpt.broadPhaseCollisionAlgorithm`:
...
For the ones like me who doesn't know what a Broad Phase Algorithm is, basically is an algorithm that can detect easily what objects have a **high possibility** of collide, so it performs less collision checks, so more performance. 

Each algorithm may work better or worse depending on the type of game, so choose carefully! 

## Thanks

Time for saying goodbye! Thank you reader for your time, and thanks to all the contributors, specially, the ones who made possible this relaese, @dragoncoder047, @mflerackers, @milosilo-dev and @anthonygood. 
