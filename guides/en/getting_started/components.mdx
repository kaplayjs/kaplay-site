...
- `sprite()` to make the object draw an image, and play
  animations
- `area()` to make the object do collision detection, with
  methods like `onCollide()`
- `text()` to make the object draw text
- `scale()` to make the game object bigger or smaller or to
  stretch it
