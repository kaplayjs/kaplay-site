...
`GetOpt.recursive` makes it work recursively to get all children and their descendants.

<Code
    code={`
bag.get("*"); // Get all children [2]
// We can pass recursive: true for getting descendants too
bag.get("favorite", { recursive: true }); // The favorite! [superMiniBag] 
    `}
    pgCode={`
kaplay();
...
One way for create a game object dynamically is by creating a function that returns
