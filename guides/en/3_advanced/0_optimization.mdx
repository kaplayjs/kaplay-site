...
-   Text: `text()` or `drawText()` transform instead of creating a game object
    for each letter/word.
-   Data: `const gameData = {}` for storing game state instead of creating a
    game object the state.
