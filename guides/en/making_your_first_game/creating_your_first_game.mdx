...
If you need any help, you can find the source code at the [end](#full-game-code-here).
...
- `move()` makes it move towards a direction infinitely. In this case we move
  towards the `LEFT` by `240` pixels per second
...
scene("game", () => {
    ...
    onKeyPress("space", () => jump(bean));
    onClick(() => jump(bean));

    // start spawning trees
    spawnTree();

    // lose if bean collides with any game obj with tag "tree"
    bean.onCollide("tree", () => {
