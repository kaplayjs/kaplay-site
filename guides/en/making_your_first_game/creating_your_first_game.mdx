...
You can find the source code at the [end](#full-game-code-here) or in the [KAPLAYGROUND](https://play.kaplayjs.com/?example=runner)!
...
decide if it should be subject to gravity, some components might decide what color
...
- `area()` gives it a collider area, so we can check for collisions with other
  characters later on
- `body()` gives it a physical body, making it fall due to gravity and giving it the ability
  to jump
...
`setGravity()` define the gravity of your world, everything with a `body()` will be falling down.
...
Pretty straightforward! Refresh the game and you should see our Player is now
...
for collision between Player and any object with tag "tree".
...
2. It'll be more fun if the trees spawn at different intervals. We cannot do
   that with `loop()`, but we can achieve that with recursive `wait()`s, which
   wait for x seconds before executing some code.
...
You can also play the result in the [KAPLAYGROUND](https://play.kaplayjs.com/?example=runner)!
