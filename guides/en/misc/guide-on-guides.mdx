...
Display extra or useful information. You can import this component:
...
Render JavaScript code with an option to Open in KAPLAYGROUND. You can import this component:
...
There's situations that the code in `code` is very similar to the one in
`pgCode`, just without the initial `kaplay()` call, so you can use some options:

| Options                   | Works for                                                                                                                      |
|---------------------------|--------------------------------------------------------------------------------------------------------------------------------|
| `addKAPLAYCall`           | If no `pgCode`, it will use `code` inserting `kaplay()` in the start                                                           |
| `loadCrew(["crewAsset"])` | Load sprites from crew in the start of the example. Just works if `addKAPLAYCall` is true, and just work with sprites for now. |
